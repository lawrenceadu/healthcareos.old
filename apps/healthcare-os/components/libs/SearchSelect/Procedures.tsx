...
function Procedures({ value, isMulti, disabled, onChange }: ProcedureProps) {
  /**
   *
   * @param search
   * @param callback
  ...
  const loadOptions = debounce(
    (search: string, callback: (options: any) => void) => {
      http
        ...
        .then(({ procedures }: { procedures: ProcedureModel[] }) => {
          callback(procedures.map((i) => ({ label: i.name, value: i.id })));
        });
    },
    500
  );
  ...
  return (
    <AsyncSelect
      ...
      noOptionsMessage={({ inputValue }) => {
        if (inputValue) {
          return 'No procedure matches your search query';
        } else {
          return 'Start typing to search for a procedure';
        }
      }}
    />
  );
