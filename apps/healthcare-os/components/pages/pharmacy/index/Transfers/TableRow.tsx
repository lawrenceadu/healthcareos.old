...
export interface TableRowProps {
  transfer: MedicineTransferModel;
  mutate: () => void;
}
...
function TableRow({ transfer, mutate }: TableRowProps) {
  ...
  const handleDelete = () =>
    Confirm({
      ...
      if (proceed) {
        api
          .deleteMedicineTransferService(transfer.id)
          .then(() => {
            mutate();
  ...
  const handleStatusUpdate = (status: MedicineTransferModel['status']) =>
    Confirm({
      ...
      if (proceed) {
        api
          .updateMedicineTransferStatusService({ status }, transfer.id)
          .then(() => {
