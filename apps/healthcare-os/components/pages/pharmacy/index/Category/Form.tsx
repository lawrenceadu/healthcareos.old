      ...
      >
        <Formik
          ...
          onSubmit={(data, { setSubmitting, setErrors }) => {
            if (params) {
              api
                .updateMedicineCategoryService(data, params.id)
                .then(() => {
                  toast.success('Medicine category updated');
                  setShow(false);
                  mutate?.();
                })
            ...
            if (!params) {
              api
                .createMedicineCategoryService(data)
                .then(() => {
                  toast.success('Medicine category created');
                  setShow(false);
                  mutate?.();
                })
                .catch((error) => setErrors(error?.fields || {}))
