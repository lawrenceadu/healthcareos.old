...
import { ItemPurchaseModel } from '../../../../../models';
...
function TableRow({ purchase, mutate }: TableRowProps) {
  ...
  const handleDelete = () =>
    Confirm({
      ...
      if (proceed) {
        api
          .deleteItemPurchaseService(purchase.id)
          .then(() => {
            mutate?.();
  ...
  const handleStatusUpdate = (status: ItemPurchaseModel['status']) =>
    Confirm({
      header: 'Update status',
      message: (
        <>
          Are you sure you want to mark this purchase as <b>{status}</b>?
        </>
      ),
      buttons: {
      ...
      if (proceed) {
        api
          .updateItemPurchaseStatusService({ status }, purchase.id)
          .then(() => {
        ...
        <td colSpan={9} className="!p-0">
          <table>
            ...
            <tbody>
              ...
              {purchase.notes && (
                <tr>
                  <td colSpan={5} className="!whitespace-normal">
